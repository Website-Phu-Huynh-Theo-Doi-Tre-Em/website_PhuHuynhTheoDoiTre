...
public partial class website_PhuHuynhTheoDoiTre_web_module_web_QuenMatKhau : System.Web.UI.Page
{
    ...
    protected void btnGui_ServerClick(object sender, EventArgs e)
    {
        ...
        var checkMail = from hs in db.tbHocSinhs
                        where (hs.hocsinh_emailme == mail || hs.hocsinh_emaillba == mail) && hs.hocsinh_tinhtrang == null
                        select hs;
        if (checkMail.Count() == 1)
        {
            ...
            db.SubmitChanges();
            ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "Alert", "swal('Gửi Email thành công! Vui lòng xác nhận và đăng nhập','','success').then(function(){parent.location.href='/website_PhuHuynhTheoDoiTre/web_module/web_Login.aspx';})", true);
        }
